               ...
               <div className={`w-2.5 h-2.5 rounded-full ${status?.ss.active ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`} />
                ...
                {status?.ss.profile && (
                  <span className="text-xs bg-gray-700 px-2 py-0.5 rounded text-gray-300 ml-2">
                    Current: {status.ss.profile.toUpperCase()} {PROFILE_FLAGS[status.ss.profile as Profile]}
                  </span>
                )}
            </div>
              ...
              <div className="flex space-x-3">
                <div className="relative flex-grow">
                  <select
                    ...
                    className="w-full appearance-none bg-gray-800 border border-gray-600 text-white py-2.5 px-4 pr-8 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent cursor-pointer"
                  >
                    {PROFILES.map((p) => (
                      <option key={p} value={p}>
                         {p.toUpperCase()} - {PROFILE_FLAGS[p]} Node
                      </option>
                    ))}
                  </select>
